...
def generate_sslcommerz_payment(request,order):
    post_data = {
        ...
        'success_url': request.build_absolute_uri(f'/order/payment/success/{order.id}/'),
        'fail_url': request.build_absolute_uri(f'/order/payment/fail/{order.id}/'),
        'cancel_url': request.build_absolute_uri(f'/order/payment/cancel/{order.id}/'),
...
def send_order_confirmation_email(order):
    ...
    message = render_to_string('email/order_confirmation.html', {'order' : order}) # html code ke --> string e convert kore
    to = order.email
